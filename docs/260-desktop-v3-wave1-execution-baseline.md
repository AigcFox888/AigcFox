...
当前首次安装包仍由 GitHub Actions `desktop-v3-package.yml` 产出并经由维护者上传到七牛对象存储（Kodo）或自有 HTTPS 下载源；artifact 内的 `release-manifest.json` 与 `SHA256SUMS.txt` 固定作为首装包文件清单与校验真相。已安装用户后续版本的更新策略固定为在线更新，不重复下载安装包，且运行中的客户端不强制打断，命中强更时只在下一次启动生效。
