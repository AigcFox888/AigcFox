...
| 交付平台边界 | `Windows + macOS` 是当前正式打包目标 | `.github/workflows/desktop-v3-package.yml` 只允许在 GitHub Actions 内产出 `windows-latest + macos-latest` 构件；`.github/workflows/desktop-v3-ci.yml` 继续固定在 `ubuntu-24.04` 做治理、测试与 smoke proof，不再上传 Linux 终端用户安装包；脚本 `qa:desktop-v3-linux-package` 已明确排除在当前验收矩阵之外，本地 `WSL` 只做 renderer / host / QA proof | workflow / baseline |
