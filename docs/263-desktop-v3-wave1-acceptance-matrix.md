...
| 聚合验收入口 | Wave 1 聚合验证入口成立 | 在 `Windows + PowerShell` 当前默认宿主链上，`pnpm qa:desktop-v3-wave1-readiness` 作为聚合入口成立，并负责串行收口 `source-of-truth document gate / lint / typecheck / test / cargo test / build / responsive smoke / tauri dev smoke / tauri build / packaged app smoke` 的摘要与归档路径 | `output/verification` summary |
