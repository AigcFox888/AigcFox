...
| 维护者分发路径 | 中国区用户首次安装不直接从 GitHub 获取安装包 | `desktop-v3-package.yml` 只给维护者产出 `Windows + macOS` 完整安装包 artifacts；维护者必须从 GitHub Actions 下载 bundle，并上传到七牛对象存储（Kodo）或自有 HTTPS 下载源后，再把国内可达首次下载地址或离线包分发给用户；不直接下发 GitHub artifact URL 或 GitHub Releases URL | workflow / runbook |
