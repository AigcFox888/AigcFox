...
| 交付平台边界 | `Windows + macOS` 是当前正式打包目标 | `.github/workflows/desktop-v3-package.yml` 只允许在 GitHub Actions 内产出 `windows-latest + macos-latest` 构件；其中 Windows job 必须先显式预装并导出固定 `WiX Toolset 3.14.1`，并把 Windows bundle 目标固定收敛到 `msi`，避免把 `tauri build` 绑定到 WiX 或 NSIS 的单次在线下载；`.github/workflows/desktop-v3-ci.yml` 继续固定在 `ubuntu-24.04` 做治理、测试与 smoke proof，不再上传 Linux 终端用户安装包；脚本 `qa:desktop-v3-linux-package` 已明确排除在当前验收矩阵之外，本地 `WSL` 只做 renderer / host / QA proof | workflow / baseline |
| 维护者分发路径 | 中国区用户首次安装不直接从 GitHub 获取安装包 | `desktop-v3-package.yml` 只给维护者产出 `Windows + macOS` 完整安装包 artifacts，并附带 `release-manifest.json` 与 `SHA256SUMS.txt` 作为首装包文件清单与校验真相；维护者必须从 GitHub Actions 下载 bundle，按清单核对后上传到七牛对象存储（Kodo）或自有 HTTPS 下载源，再把国内可达首次下载地址或离线包分发给用户；不直接下发 GitHub artifact URL 或 GitHub Releases URL | workflow / runbook |
| 后续更新策略 | 已安装用户后续版本不重复下载安装包 | updater 结构化重写后，客户端更新源固定为七牛对象存储（Kodo）或自有 HTTPS；运行中的客户端不强制打断，命中强更时只在下一次启动生效；当前代码仍未落地 updater 实现 | baseline / governance |
...
| 更新边界 | 不作为客户端更新源 | GitHub 产物不作为客户端更新源；正式更新只走七牛对象存储（Kodo）或自有 HTTPS，且已安装用户后续不重复下载安装包 | baseline / delivery docs |
