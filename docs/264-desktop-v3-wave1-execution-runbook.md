...
- 当前 `desktop-v3-package.yml` 的 Windows job 还必须先显式预装并导出固定 `WiX Toolset 3.14.1`，并把 Windows bundle 目标固定在 `msi` 单入口，避免把 MSI 打包稳定性绑到 `tauri build` 内部对 GitHub release 或 NSIS 压缩包的单次在线下载；如果 Windows 包失败，优先先看 WiX 预装步骤与 package job 日志，而不是先怀疑 renderer / Rust skeleton
