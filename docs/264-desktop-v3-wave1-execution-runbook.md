...
- 当前 `desktop-v3-package.yml` 的 Windows job 还必须先显式预装并导出固定 `WiX Toolset 3.14.1`，同时把必需的 WiX 二进制预热到 `%LOCALAPPDATA%\tauri\WixTools314`，再把 Windows bundle 目标固定在 `msi` 单入口；否则 `tauri build` 仍会退回到对 `wix314-binaries.zip` 的单次在线下载并在 GitHub 网络抖动时失败。如果 Windows 包失败，优先先看 WiX 预装 / `WixTools314` 预热步骤与 package job 日志，而不是先怀疑 renderer / Rust skeleton
